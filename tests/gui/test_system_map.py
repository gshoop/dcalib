"""Tests for the System Map widgets in ``dcalib.gui.system_map``.
...
model's cell. Also covers the colour bar (``dcalib.gui.map_colors``) and the
...
from dcalib.gui._flow_layout import FlowLayout
...
from dcalib.gui.system_map import (
...
def _ok(sigma: float, **extra: float) -> ChannelView:
    return ChannelView(
        STATUS_OK, metrics={"n_events": 1000.0, "cv_gain": sigma, "peak_spread": 0.01, **extra}
    )
...
def _sample_views() -> dict[ChannelTuple, ChannelView]:
    ...
    Board (3, 17) also has its cathodes' calibration status (cathode 1 uncalibrated).
    """
    ...
    views[_anode(NODE, BOARD, 2)] = ChannelView(
        STATUS_OK, (FLAG_CONVEX_CURVE,), metrics={"cv_gain": 12.0}
    )
    ...
    views[_anode(NODE, BOARD, 5)] = ChannelView(
        STATUS_OK, options_source="override", metrics={"cv_gain": math.nan}
    )
    views[_cathode(NODE, BOARD, 1)] = ChannelView(CATHODE_UNCALIBRATED)
...
class TestPanelGrid:
    ...
    def test_cell_colours(self, grid: PanelGridWidget) -> None:
        ...
        assert image.pixelColor(_center(grid.cell_rect(NODE, BOARD, 1))) == QColor(
            CATEGORY_COLORS[CATEGORY_CATHODE_UNCALIBRATED]
        )
        assert image.pixelColor(_center(grid.cell_rect(NODE, BOARD, 2))) == QColor(
            CATEGORY_COLORS[CATEGORY_CATHODE_CALIBRATED]
        )
...
class TestSystemMapWidget:
    ...
    def test_colour_mode_switch(self, qtbot: QtBot, map_widget: SystemMapWidget) -> None:
        grid = _grid1(map_widget)
        views_before = [c.view for c in _model(map_widget).boards[(NODE, BOARD)].anodes]

        with (
            qtbot.assertNotEmitted(map_widget.color_mode_changed),
            qtbot.assertNotEmitted(map_widget.channel_selected),
        ):
            map_widget.set_color_mode("cv_gain")
        model = _model(map_widget)
        assert model.color_mode == "cv_gain"
        # Same views (nothing re-read), new fills.
        board = model.boards[(NODE, BOARD)]
        assert [c.view for c in board.anodes] == views_before
        limits = model.limits_for("anode")
        assert limits is not None
        assert board.anodes[5].fill == metric_color(16.0, limits)
        # NaN-valued (failed) anode 3 is grey.
        assert board.anodes[2].fill == QColor(NOT_FITTED_COLOR)
        # Cathodes keep their calibration colours and have no limits.
        assert model.limits_for("cathode") is None
        map_widget.set_view(VIEW_CATHODES)
        point = _center(grid.cell_rect(NODE, BOARD, 3))
        status_fill = QColor(CATEGORY_COLORS[CATEGORY_CATHODE_CALIBRATED])
        assert grid.grab().toImage().pixelColor(point) == status_fill

        # The colour bar shows the metric and the anode limits.
        spec = METRIC_SPECS["cv_gain"]
        map_widget.set_view(VIEW_ANODES)
        assert not map_widget.color_bar.isHidden()
        assert map_widget.color_bar.name_label.text() == "CV gain:"
        assert map_widget.color_bar.limits == limits
        ...
        assert map_widget.legend_labels()[-1] == OVERRIDE_LEGEND_LABEL
        assert not map_widget.override_legend.isHidden()
        assert map_widget.summary_label.text().startswith("CV gain | Anodes: median")
    ...
    def test_combo_emits_colour_mode_changed(
        ...
        with qtbot.waitSignal(map_widget.color_mode_changed, timeout=1000) as blocker:
            map_widget.color_mode_combo.setCurrentIndex(COLOR_MODES.index("peak_spread"))
        assert blocker.args == ["peak_spread"]
        assert map_widget.color_mode == "peak_spread"
        assert _model(map_widget).color_mode == "peak_spread"
        map_widget.set_color_mode("fwhm_511_after")
        assert map_widget.color_mode_combo.currentData() == "fwhm_511_after"

    def test_metric_mode_before_state(self, qtbot: QtBot) -> None:
        widget = SystemMapWidget()
        qtbot.addWidget(widget)
        widget.set_color_mode("ge_cs_max_diff")
        assert not widget.color_bar.isHidden()
        assert widget.color_bar.min_label.text() == "n/a"
        widget.set_state(_sample_views(), active_boards=ACTIVE_BOARDS)
        assert _model(widget).color_mode == "ge_cs_max_diff"
        assert widget.summary_label.text() == "Ge/Cs difference | Anodes: no values"
    ...
    def test_informational_flags(self, map_widget: SystemMapWidget) -> None:
        ...
        map_widget.set_informational_flags([FLAG_CONVEX_CURVE])
        assert map_widget.informational_flags == frozenset({FLAG_CONVEX_CURVE})
    ...
    def test_board_with_data_and_no_views(self, qtbot: QtBot) -> None:
        ...
        assert widget.summary_label.text() == (
            "Anodes: 39\u00a0no\u00a0data | Cathodes: 1\u00a0not\u00a0fitted, 7\u00a0no\u00a0data"
        )
...
class TestColorBar:
    def test_labels(self, qtbot: QtBot) -> None:
        ...
        spec = METRIC_SPECS["fwhm_511_change"]
...
def _distinct_views(node: int, board: int) -> dict[ChannelTuple, ChannelView]:
    """Every anode of ``(node, board)`` with its own CV gain (own fill in that mode).

    Cathodes keep their calibration colours in metric modes, so only the
    anodes have distinct fills.
    """
    views: dict[ChannelTuple, ChannelView] = {
        _anode(node, board, p): _ok(float(p)) for p in range(1, ANODES + 1)
    }
    views.update({_cathode(node, board, i): ChannelView(CATHODE_CALIBRATED) for i in range(1, 9)})
    return views
...
class TestPixelAccurateHits:
    ...
    cell painted in that colour. Anode fills are distinct per cell (CV gain
    mode); cathodes share their calibration colour, so only anodes are checked.
    """

    @pytest.mark.parametrize("size", [(1600, 420), (1366, 214)])
    def test_grid(self, qtbot: QtBot, size: tuple[int, int]) -> None:
        view = VIEW_ANODES
        widget = SystemMapWidget()
        qtbot.addWidget(widget)
        widget.resize(*size)
        widget.set_color_mode("cv_gain")
        ...
        tile = grid.tile_rect(NODE, BOARD)
        assert tile.width() / len(cells) < 5  # narrow anode cells
        image = grid.grab().toImage()
        y = int(tile.bottom()) - 1
    ...
    def test_strip(self, qtbot: QtBot, width: int) -> None:
        ...
        model = build_system_map(_distinct_views(NODE, BOARD), color_mode="cv_gain")
        board = model.boards[(NODE, BOARD)]
        strip.set_board(board)
        image = strip.grab().toImage()
        anode_fills = {cell.fill.rgb(): cell for cell in board.anodes}
        assert len(anode_fills) == ANODES
        # Only the anodes: stop in the middle of the empty slot before the cathodes.
        ...
        for x in range(int(split)):
            painted = anode_fills.get(image.pixelColor(x, y).rgb())
            if painted is None:
                continue
            received.clear()
            ...
            assert received == [painted.channel], f"x={x}"
            checked += 1
        assert checked > 0.9 * strip.width() * 39 / 48
...
class TestOverrideTick:
    ...
    def test_grid_only_when_cells_are_wide(self, grid: PanelGridWidget) -> None:
        ...
        plain = model.boards[(NODE, BOARD)].anodes[5]
        assert override.is_override and not plain.is_override
        # Narrow (~4 px) anode cells: no tick.
        ...
        # Wide enough: the tick is drawn on the override cell only.
        grid.resize(1900, 320)
        rect = grid.cell_rect(NODE, BOARD, 5)
        assert rect.width() >= 6
        image = grid.grab().toImage()
        corner = image.pixelColor(self._corner(rect))
        assert corner == QColor(OVERRIDE_TICK_COLOR) != override.fill
        assert image.pixelColor(self._corner(grid.cell_rect(NODE, BOARD, 6))) == plain.fill
...
class TestColorBarLazyLut:
    def test_lut_only_with_a_metric(self, qtbot: QtBot) -> None:
        ...
        bar.set_metric(METRIC_SPECS["peak_spread"], (0.0, 0.1))
