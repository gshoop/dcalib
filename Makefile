...
# Source trees checked by the linters (scripts/ appears in phase 2).
CODE_DIRS = src/ tests/ $(wildcard scripts/)
TYPED_DIRS = src/ $(wildcard scripts/)
...
install:  ## Install adc2kev (editable, from ADC2KEV=~/adc2kev-python) and dcalib
	$(REQUIRE_VENV)
	$(PIP) install -e $(ADC2KEV)
	$(PIP) install -e .

install-dev:  ## Install adc2kev (editable) and dcalib with dev dependencies
	$(REQUIRE_VENV)
	$(PIP) install -e $(ADC2KEV)
	$(PIP) install -e ".[dev]"

cython-check:  ## Check adc2kev's Cython parser (needed only by scripts/validate_holdout.py)
...
test-realdata:  ## Run only the real-data tests (need the full-system adc2kev cache)
	$(PYTHON) -m pytest -m realdata

test-cov:  ## Run tests with coverage (skip real-data tests)
	$(PYTHON) -m pytest -m "not realdata" --cov=dcalib --cov-report=term-missing --cov-report=html --cov-report=xml
