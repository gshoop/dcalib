...
Forked from uvcorr (``uvcorr/gui/map_colors.py``), which extended adc2kev
2.2.7's four calibration colours to status categories and metric modes. The
map has two kinds of colour mode (plan section 9, System Map row):

- **Status** (:data:`MODE_STATUS`): every anode falls into one of the anode
  categories, derived from its depth-fit status, its warning flags and its
  review state by :func:`status_category`; a rejected anode is shown as
  rejected whatever its status. Every **cathode** is coloured by its keV
  calibration (calibrated or not), in every colour mode: an anode's depth fit
  needs calibrated cathodes, and the uncalibrated ones (plan D3) are the main
  reason anodes cannot be corrected. The palette keeps uvcorr's green ok,
  orange flagged, red failed, blue too few, grey not fitted and dark no data,
  and adds purple rejected, a yellow no gain, a teal "nothing to correct",
  brown "no calibration", and a steel blue / crimson pair for the cathodes.
  The legend labels every colour, so the colour never stands alone.
- **Metric** (the keys of :data:`METRIC_SPECS`): an anode's metric value is
  mapped onto the viridis sequential colormap, with the 2nd and 98th
  percentiles of the finite values as limits (:func:`metric_limits`). NaN or
  missing values get the not-fitted grey, and anodes without events keep the
  no-data colour.
...
CATEGORY_OK = "ok"
"""Status ``ok`` (corrected) without warning flags (or only informational ones)."""

CATEGORY_FLAGGED = "flagged"
"""Status ``ok`` with at least one warning flag outside the informational set."""

CATEGORY_REJECTED = "rejected"
"""Rejected in the review (left out of the ``.dcc`` whatever its status)."""

CATEGORY_NO_GAIN = "no_gain"
"""Status ``no_gain``: the correction did not improve the held-out resolution."""

CATEGORY_NO_DEPTH = "no_depth"
"""Status ``no_depth_dependence``: a flat curve, nothing to correct."""
...
CATEGORY_NO_CALIBRATION = "no_calibration"
"""Status ``no_anode_calibration`` or ``no_calibrated_cathode``."""

CATEGORY_NOT_FITTED = "not_fitted"
"""The anode has events but no result yet."""

CATEGORY_NO_DATA = "no_data"
"""The channel (or its whole board) has no events."""

CATEGORY_CATHODE_CALIBRATED = "cathode_calibrated"
"""A cathode with a valid keV calibration."""

CATEGORY_CATHODE_UNCALIBRATED = "cathode_uncalibrated"
"""A cathode without a valid keV calibration (plan D3)."""

ANODE_CATEGORIES: tuple[str, ...] = (
    CATEGORY_OK,
    CATEGORY_FLAGGED,
    CATEGORY_REJECTED,
    CATEGORY_NO_GAIN,
...
"""Categories an anode cell can take, in legend and summary order."""
...
CATEGORY_LABELS: dict[str, str] = {
    CATEGORY_OK: "Corrected",
    CATEGORY_FLAGGED: "Flagged",
    CATEGORY_REJECTED: "Rejected",
    CATEGORY_NO_GAIN: "No gain",
    CATEGORY_NO_DEPTH: "No depth effect",
    CATEGORY_TOO_FEW: "Too few events",
    CATEGORY_FAILED: "Fit failed",
    CATEGORY_NO_CALIBRATION: "No calibration",
    CATEGORY_NOT_FITTED: "Not fitted",
    CATEGORY_NO_DATA: "No data",
    CATEGORY_CATHODE_CALIBRATED: "Cathode calibrated",
    CATEGORY_CATHODE_UNCALIBRATED: "Cathode uncalibrated",
}

# Dark-theme palette (map background #1e1e1e): uvcorr's six colours plus six.
CATEGORY_COLORS: dict[str, str] = {
    CATEGORY_OK: "#2ecc71",
    CATEGORY_FLAGGED: "#f39c12",
    CATEGORY_REJECTED: "#b76be0",
    CATEGORY_NO_GAIN: "#e8d44d",
    CATEGORY_NO_DEPTH: "#2a9d8f",
    CATEGORY_TOO_FEW: "#5d86b8",
    CATEGORY_FAILED: "#e74c3c",
    CATEGORY_NO_CALIBRATION: "#8d6e4f",
    CATEGORY_NOT_FITTED: "#555555",
    CATEGORY_NO_DATA: "#2b2b2b",
    CATEGORY_CATHODE_CALIBRATED: "#9fb8d0",
    CATEGORY_CATHODE_UNCALIBRATED: "#a8324a",
}
...
"""Fill of anodes with events but no result, and of NaN metric values."""

CATHODE_CALIBRATED = "calibrated"
CATHODE_UNCALIBRATED = "uncalibrated"
CATHODE_STATUSES: tuple[str, ...] = (CATHODE_CALIBRATED, CATHODE_UNCALIBRATED)
"""The ``status`` of a cathode's map view."""

DEFAULT_INFORMATIONAL_FLAGS: frozenset[str] = frozenset(
    {"partial_cathode_coverage", "narrow_ca_coverage", "slice_fit_failed"}
)
"""Flags that do not make a corrected anode ``flagged`` by default (tooltip only)."""
...
_STATUS_CATEGORY: dict[str, str] = {
    STATUS_NO_GAIN: CATEGORY_NO_GAIN,
    STATUS_NO_DEPTH_DEPENDENCE: CATEGORY_NO_DEPTH,
    STATUS_TOO_FEW_EVENTS: CATEGORY_TOO_FEW,
    STATUS_FIT_FAILED: CATEGORY_FAILED,
    STATUS_NO_ANODE_CALIBRATION: CATEGORY_NO_CALIBRATION,
    STATUS_NO_CALIBRATED_CATHODE: CATEGORY_NO_CALIBRATION,
}
...
def status_category(
    ...
    *,
    ...
    3. A cathode status (``calibrated`` / ``uncalibrated``) -> its category.
    4. ``review == "rejected"`` -> ``rejected``.
    5. ``ok`` with any flag not in ``informational_flags`` -> ``flagged``,
       otherwise ``ok``.
    6. Any other status -> its category (``no_gain``, ``no_depth``,
       ``too_few``, ``failed``; both calibration statuses -> ``no_calibration``).
    ...
    if status == CATHODE_CALIBRATED:
        return CATEGORY_CATHODE_CALIBRATED
    if status == CATHODE_UNCALIBRATED:
        return CATEGORY_CATHODE_UNCALIBRATED
    if status not in STATUSES:
        raise ValueError(
            f"status must be one of {STATUSES + CATHODE_STATUSES} or None, got {status!r}"
        )
...
METRIC_SPECS: dict[str, MetricSpec] = {
    spec.key: spec
    for spec in (
        MetricSpec("cv_gain", "CV gain", formatter=_format_percent),
        MetricSpec("peak_spread", "Peak spread", formatter=_format_percent),
        MetricSpec("fwhm_511_after", "FWHM 511 keV", "%", _format_points),
        MetricSpec("fwhm_662_after", "FWHM 662 keV", "%", _format_points),
        MetricSpec("fwhm_511_change", "FWHM 511 change", formatter=_format_percent),
        MetricSpec("ge_cs_max_diff", "Ge/Cs difference", formatter=_format_percent),
    )
}
"""Metric colour modes by key, in selector order.

``fwhm_511_change`` is ``fwhm_511_after / fwhm_511_before - 1`` (not a CSV
column); the others are the CSV columns of the same name (plan section 6.4).
Only anodes are coloured by a metric; cathodes keep their calibration colours.
"""
...
def legend_entries(mode: str) -> tuple[tuple[str, str], ...]:
    ...
    The status mode lists every category. A metric mode lists the two fills
    that are not on the colour bar (no value, no data) and the cathode
    calibration colours, which every mode keeps.
    ...
    if is_metric_mode(mode):
        return (
            ...
            *(
                (CATEGORY_LABELS[c], CATEGORY_COLORS[c])
                for c in (CATEGORY_CATHODE_CALIBRATED, CATEGORY_CATHODE_UNCALIBRATED)
            ),
        )
...
class ColorBarWidget(QWidget):
    ...
    are those of the anodes (cathodes are never coloured by a metric).
    """
    ...
    def set_metric(self, spec: MetricSpec | None, limits: tuple[float, float] | None) -> None:
        ...
        self.setToolTip(
            ...
            "of the anodes. Grey: no value; dark: no data. Cathodes keep their "
            "calibration colours."
        )
