...
Forked from uvcorr (``uvcorr/gui/system_map.py``), itself forked from adc2kev
2.2.7 ``gui/system_map.py``; keep the structure close to them so fixes can be
carried across (uvcorr's docstring lists its changes from adc2kev: views,
colour modes with a colour bar, ``set_selection``, the flow-layout top row,
fill-aware labels, override dog-ears, ``view_changed`` / ``update_views`` and
the pixel hit-testing fix). Changes for dcalib are in the model
(:mod:`dcalib.gui._system_map_model`) and the colours
(:mod:`dcalib.gui.map_colors`): anode statuses, review and metrics, and
cathodes coloured by their keV calibration in every mode.
...
# the model (see dcalib.gui.map_colors).
...
class SystemMapWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.color_mode_combo.setToolTip(
            "Colour the anodes by depth-fit status or by a metric (viridis scale, "
            f"clipped to the {clip_range_text()}); cathodes always show their keV "
            "calibration"
        )
        ...
        self.override_legend.setToolTip(
            "Corner tick: the anode was re-fitted with its own options (override)"
        )
    ...
    def set_state(
        ...
        :func:`~dcalib.gui._system_map_model.build_system_map` for the
        argument semantics.
        """
    ...
    def update_views(self, changed: Mapping[ChannelKeyT, ChannelView]) -> None:
        ...
        :func:`~dcalib.gui._system_map_model.update_system_map`), the rest
    ...
    def _is_anode(self, channel: ChannelAddress) -> bool:
        """Whether the model draws ``channel`` as an anode (True without a model)."""
        if self._model is None:
            return True
        cell = self._model.cell(channel)
        return cell is None or cell.kind == "anode"
    ...
    def build_context_menu(self, node: int, board: int, channel: ChannelTuple | None) -> QMenu:
        ...
        With an anode the menu holds "Fit Channel", a separator and "Fit
        Board"; without one (or on a cathode, which is never fitted) only "Fit
        Board". Triggering an action emits
        ...
        """
        menu = QMenu(self)
        if channel is not None and self._is_anode(as_address(channel)):
